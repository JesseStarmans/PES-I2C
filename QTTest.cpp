...
int checkReceived(std::string received) {
	if (received == "Error" ) {
		...
		else if (received.find("RDruk:") != std::string::npos){// || received == "Rruk: 2"){
		...
		else if (received.find("912071d") != std::string::npos) {
			{
			SocketClient client(7070, PI_B);
			client.sendData(received);
			}
			if (received.find("Voordeur: ") != std::string::npos) {
				SocketClient QTClient(9090, QT);
				QTClient.sendData("Voordeur: open");
			}
			else {
		...
		else if (received.find("b8c72833") != std::string::npos) {
			{
			SocketClient client(7070, PI_B);
			client.sendData(received);
			}
		...
		else if (received.find("RPlant:") != std::string::npos){
			SocketClient QTClient(9090, QT);
			QTClient.sendData(received);
			return 0xD2;
		}
		// else if (received.find("RPlnt:") != std::string::npos) {		//Oude versie, werkte niet
		// 	SocketClient QTClient(9090, QT);
		// 	QTClient.sendData(received);
		// 	return 0xD3;
		// }
